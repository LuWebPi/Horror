...
function KeyItem({ cell, floor, index, collected, onCollect }: {
  cell: [number, number]; floor: number; index: number; collected: boolean; onCollect: () => void
}) {
  const groupRef = useRef<THREE.Group>(null)
  const [x, y, z] = cellToWorld(cell[0], cell[1], floor)
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      ...
      groupRef.current.position.y = y + 1.1 + Math.sin(t * 2 + index) * 0.15
    }
    ...
    if (pickupCheck.current > 0.1) {
      ...
      const dy = entities.player.y - y
      if (dx * dx + dz * dz + dy * dy < 1.6 * 1.6) {
        onCollect()
      }
    }
  })
  ...
  return (
    <group ref={groupRef} position={[x, y + 1.1, z]}>
      <mesh rotation={[Math.PI / 2, 0, 0]}>
        <torusGeometry args={[0.12, 0.035, 8, 24]} />
        <meshStandardMaterial color="#ffcc33" emissive="#ffaa00" emissiveIntensity={0.8} metalness={0.9} roughness={0.3} />
      </mesh>
      ...
      <pointLight ref={lightRef} color="#ffcc44" intensity={2} distance={3} decay={2} />
    </group>
  )
}
...
export function Keys() {
  ...
  const handleCollect = (i: number) => {
    ...
    if (remaining <= 0) {
      showMessage('All keys found. The front door is unlocked...', 4000)
      unlockExit()
  ...
  const items = useMemo(() => KEY_CELLS.map((k, i) => ({ ...k, i })), [])

  return (
    <group>
      {items.map(({ cell, floor, i }) => (
        <KeyItem
          key={i}
          cell={cell}
          floor={floor}
